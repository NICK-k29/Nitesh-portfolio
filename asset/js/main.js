                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            var result, cls;
                            var responseText = (typeof msg === 'string') ? msg.trim() : '';

                            if (responseText === 'Success') {
                                result = 'Thank you! Your message has been sent successfully.';
                                cls = 'msg-success';
                            } else {
                                result = 'Error sending email. Please try again later or contact directly.';
                                cls = 'msg-error';
                            }

                            var alertBox = $('<div class="custom-centered-alert ' + cls + '">' +
                                '<div class="alert-content">' +
                                '<h4 class="alert-title">' + (cls === "msg-success" ? "Success!" : "Oops!") + '</h4>' +
                                '<p class="text-body-1">' + result + '</p>' +
                                '<button class="close-alert text-body-2 fw-medium">Close</button>' +
                                '</div></div>');

                            $('body').append(alertBox);

                            $('.custom-centered-alert .close-alert').on('click', function () {
                                $(this).closest('.custom-centered-alert').fadeOut(300, function () {
                                    $(this).remove();
                                });
                            });
                        ...
                        error: function () {
                            var alertBox = $('<div class="custom-centered-alert msg-error">' +
                                '<div class="alert-content">' +
                                '<h4 class="alert-title">Oops!</h4>' +
                                '<p class="text-body-1">Error sending email. Please ensure the site is running on a server (PHP required).</p>' +
                                '<button class="close-alert text-body-2 fw-medium">Close</button>' +
                                '</div></div>');
